  ...
  if (child) {
    ...
    switch (childType) {
      ...
      case VObjectType::slConstant_assignment_pattern_expression: {
        m_valueFactory.deleteValue(value);
        value = evalExpr(fC, fC->Child(child), instance, muteErrors);
        break;
      }
      case VObjectType::slAssignment_pattern: {
        NodeId Structure_pattern_key = fC->Child(child);
        char base = 'h';
        std::string svalue;
        uint64_t hex_value = 0;
        while (Structure_pattern_key) {
          auto key = fC->SymName(fC->Child(Structure_pattern_key));
          // TODO: use key to determine order
          NodeId Constant_expression = fC->Sibling(Structure_pattern_key);
          ...
          Structure_pattern_key = fC->Sibling(Constant_expression);
        }
